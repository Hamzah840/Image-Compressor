...
ini_set('display_errors', 0);
	...
	function upload_compress_jpeg($compression=60) {		//Default compression is 60%
		$min = "min-";
		$prmtName = $_FILES["fileToUpload"]["name"];
		$tempName = $_FILES["fileToUpload"]["tmp_name"];
		$upload_dir = "./img/compressor/uploads_jpeg/";
		$compressed_dir = "./img/compressor/compressed_jpeg/";
		...
		if ($_FILES["fileToUpload"]["size"] > 50000000) {       // 50 MB Maximum File size is allowed
		  echo "<div class='error fa'>Sorry, your file is too large</div>";
		  $uploadOk = 0;
		}
		...
		if($imageFileType == "jpeg" || $imageFileType == "jpg") {
		  ...
		  echo "<div class='error fa'>Sorry, only JPEG & JPG Format is allowed</div>";
		  $uploadOk = 0;
		}
		...
		// if everything is ok, try to upload file
		  ...
		  if (move_uploaded_file($tempName, $target_file)) {
				$filePath = "./img/compressor/uploads_jpeg/".htmlspecialchars($prmtName);
				$compressedFilePath = "./img/compressor/compressed_jpeg/".$min.$prmtName;
	...
	function src_file_loc($filename) {
		return "./img/compressor/uploads_jpeg/".htmlspecialchars($filename);
	}

	function compressed_file_loc($filename) {
		return "./img/compressor/compressed_jpeg/".$filename;
	}
